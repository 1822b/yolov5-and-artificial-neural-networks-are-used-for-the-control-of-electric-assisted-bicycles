...
os.environ["QT_QPA_PLATFORM_PLUGIN_PATH"] = "/home/wxh/.local/lib/python3.9/site-packages/cv2/qt/plugins"
...
with open("video_detect_test.csv", "w") as file:
    file.write("Timestamp,Speed(km/h),BPM,Slope(%),PWM\n")
    elapsed_time = 0.00
...
class MotorController:
    ...
    def control_motor(self, speed, bpm, yaw):
        ...
        file.write(f"{elapsed_time:.2f},,{speed:.2f},{yaw:.2f},{pwm_value_1:.2f}\n")
...
### 综合功能 ###
class VideoDetectionControl(QMainWindow):
    ...
    def update_frame(self):
        ...
        if success:
            ...
            # 图像处理和显示
            img_resized = cv2.resize(img0, (self.video_label.width(), self.video_label.height()))
...
if __name__ == "__main__":
    ...
    moter_id = None
    motor_controller = MotorController(p_pwm, interpreter,moter_id)  # 初始化电机控制器
    # 创建 VideoDetectionControl 实例
    window = VideoDetectionControl(yolo_model, label_dict, motor_controller, interpreter)
    window.show()
    sys.exit(app.exec_())
